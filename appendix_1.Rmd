---
title: "Appendix 1: An update to the Birds of the Nebraska National Forest"
...
We are also going to import and reformat eBird data. Note that for *Junco*, we are only using populations predesignated to subspecies since roughly 57% of western *Junco* are *hyemalis* in Nebraska; thus, we can't make broad assumptions. To contrast, 98% in eastern Nebraska are *hyemalis*.
...
We can also combine the lists from 1993 and 2023 to understand what has changed.
...
The length of the list derived from Bray is `r length(unique(bray$Species))` spp., whereas the list derived from GBIF is `r length(unique(ne_nf_gbif$species))` spp.
...
Most of these species are water birds. Many species were missing from the direct GBIF download, for reasons that are unknown. These include *Astur cooperii*.
